...
import 'package:mentalsustainability/pages/Home/home_page.dart'
    hide Event; // Hide Event from home_page
...
class ApiService extends GetxService {
  ...
  Future<Map<String, dynamic>?> register(
      String universityId, String username, String password) async {
    print(
        'WARNING: register() is deprecated. Use registerWithOTP() for new OTP-based flow.');
  ...
  Future<Map<String, dynamic>?> registerWithOTP(
    ...
    try {
      ...
      print(
          'Register with OTP response: ${response.statusCode} - ${response.body}');
  ...
  Future<Map<String, dynamic>?> verifyUser(
      String universityId, int otpValue) async {
    try {
      final response = await http.post(
  ...
  Future<Map<String, dynamic>?> resetPassword(
      String universityId, int otpValue, String newPassword) async {
    try {
      final response = await http.post(
  ...
  Future<List<ApiEvent>> getEvents() async {
    try {
      ...
      print(
          'Token available: ${token.length > 20 ? "Yes (${token.substring(0, 20)}...)" : "Yes"}');
      ...
      try {
        ...
        final events = eventsData.map((eventJson) {
          ...
          return ApiEvent(
            ...
            points: eventJson['points'] != null
                ? int.tryParse(eventJson['points'].toString())
                : 50,
          );
        }).toList();
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ApiEvent(
          ...
          points: eventData['points'] != null
              ? int.tryParse(eventData['points'].toString())
              : 50, // Add points mapping
        );
      } else {
  ...
  Future<bool> createEvent(ApiEvent event) async {
    try {
      ...
      try {
        ...
        print(
            'Error parsing times in createEvent - fromTime: ${event.fromTime}, toTime: ${event.toTime} - $e');
        return false;
      }
      ...
      final response = await http.post(
        ...
        body: json.encode({
          ...
          'toTime':
              toDateTime.toIso8601String(), // Note: lowercase 't' for create
  ...
  Future<bool> registerForEvent(String userId, String eventId) async {
    try {
      ...
      print(
          'Registering user for event with: User ID=$userId, Event ID=$eventId');
  ...
  Future<List<ApiEvent>> getUserRegisteredEvents() async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
            '${baseUrl}events/user-event?query=eventsForUser&user_id=$userId'),
        headers: {
          'Authorization': token,
          'Content-Type': 'application/json',
        },
      ...
      if (response.statusCode == 200) {
        try {
          ...
          if (responseData is Map && responseData.containsKey('result')) {
            ...
            print(
                'Found result array with ${eventIds.length} event registrations');
          } else {
            print('Unexpected response format: $responseData');
            return [];
          }
          ...
          for (var eventData in eventIds) {
            ...
            try {
              ...
              if (eventDetails != null) {
                ...
                print(
                    'Successfully fetched details for event $eventId: ${eventDetails.title}');
              } else {
                print(
                    'Could not fetch details for event $eventId, creating placeholder');
          ...
          print(
              'Successfully processed ${registeredEvents.length} registered events');
        ...
        print(
            'Failed to load user registered events: ${response.statusCode} - ${response.body}');
        return [];
      }
    } catch (e) {
...
// Add a debug method to check what user ID is being used
  ...
  // Admin only function
  Future<bool> verifyEventParticipation(
      String userId, String eventId, bool participated) async {
    try {
      final token = await _authService.getToken();
      ...
      print(
          'Verify event participation response: ${response.statusCode} - ${response.body}');
    ...
    try {
      ...
      return [
        AdminEvent(
          ...
          description:
              'Join us for a tree plantation drive to help the environment.',
          date: '12/06/2023',
          fromTime: '9:00',
          toTime: '12:00',
          location: 'City Park',
  ...
  Future<List<AdminUpdate>> getAdminUpdates() async {
    ...
    return [
      AdminUpdate(
        ...
        message:
            'Tree Plantation Drive registration is now open. Register before June 3rd.',
        time: '2 hours ago',
        isRead: false,
      ),
      AdminUpdate(
  ...
  Future<bool> updateEvent(
    ...
    try {
      ...
      try {
        ...
        print(
            'Error parsing times - fromTime: $fromTime, toTime: $toTime - $e');
        return false;
      }
      ...
      final requestBody = {
        ...
        'ToTime': toDateTime
            .toIso8601String(), // Full datetime with capital T as expected by API
        'eventVenue': location,
  ...
  Future<List<Map<String, dynamic>>> getEventRegisteredUsers(
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse(
            '${baseUrl}events/user-event?query=usersForEvents&event_id=$eventId'),
        headers: {
          'Authorization': token,
          'Content-Type': 'application/json',
        },
      ...
      print(
          'Get registered users response: ${response.statusCode} - ${response.body}');
  ...
  Future<bool> verifyUserAttendance(
      String eventId, String userId, bool verified) async {
    try {
      final token = await _authService.getToken();
      ...
      print(
          'Verify attendance response: ${response.statusCode} - ${response.body}');
  ...
  Future<bool> postNotification(String title, String message) async {
    try {
      ...
      print(
          'Post notification response: ${response.statusCode} - ${response.body}');
  ...
  Future<bool> deleteNotification(dynamic notificationId) async {
    try {
      ...
      print(
          'Deleting notification with ID: $id (converted from $notificationId)');
  ...
  Future<bool> softDeleteEvent(String id) async {
    try {
      ...
      print(
          'Soft delete event response: ${response.statusCode} - ${response.body}');
  ...
  Future<bool> hardDeleteEvent(String id) async {
    try {
      ...
      print(
          'Hard delete event response: ${response.statusCode} - ${response.body}');
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData.containsKey('result') &&
          ...
          if (usersList.isNotEmpty) {
            ...
            print(
                'isVolunteer: ${usersList[0]['isVolunteer']} (type: ${usersList[0]['isVolunteer'].runtimeType})');
            print(
                'isWishVolunteer: ${usersList[0]['isWishVolunteer']} (type: ${usersList[0]['isWishVolunteer'].runtimeType})');
          }
  ...
  Future<bool> deleteUser(String universityId) async {
    try {
      ...
      final response = await http.delete(
        ...
        body: json.encode({'university_id': universityId}),
      );
  ...
  Future<Map<String, dynamic>> wishToBeVolunteer(String universityId) async {
    try {
      ...
      print(
          'Wish to be volunteer response: ${response.statusCode} - ${response.body}');
  ...
  Future<bool> makeVolunteer(String universityId) async {
    try {
      ...
      print(
          'Make volunteer response: ${response.statusCode} - ${response.body}');
  ...
  Future<bool> removeVolunteer(String universityId) async {
    try {
      ...
      print(
          'Remove volunteer response: ${response.statusCode} - ${response.body}');
  ...
  Future<bool> rejectVolunteerRequest(String universityId) async {
    try {
      ...
      print(
          'Reject volunteer request response: ${response.statusCode} - ${response.body}');
...
// Get top volunteers
  Future<List<Map<String, dynamic>>> getTopVolunteers() async {
    try {
      ...
      if (response.statusCode == 200) {
        final responseData = json.decode(response.body);
        if (responseData.containsKey('topVolunteers') &&
            responseData['topVolunteers'] is List) {
...
// Get recent participations for a user
  Future<List<Map<String, dynamic>>> getRecentParticipations(
      String userId) async {
    try {
      final token = await _authService.getToken();
      ...
      if (response.statusCode == 200) {
        final responseData = json.decode(response.body);
        if (responseData.containsKey('Participations') &&
            responseData['Participations'] is List) {
          return List<Map<String, dynamic>>.from(
              responseData['Participations']);
        }
        return [];
      } else {
  ...
  Future<Map<String, dynamic>> getVolunteerStatus(String universityId) async {
    try {
      ...
      print(
          'Get volunteer status response: ${response.statusCode} - ${response.body}');
      ...
      if (response.statusCode == 200) {
        try {
          ...
          if (responseData is List) {
            ...
            eventsData = responseData;
            print(
                'Got cached event updates (direct array): ${eventsData.length} items');
          } else if (responseData is Map &&
              responseData.containsKey('result')) {
            // Wrapped format when not cached
            eventsData = responseData['result'] as List<dynamic>;
            print(
                'Got fresh event updates (wrapped): ${eventsData.length} items');
          } else {
            print('Unexpected response format: $responseData');
            return [];
          }
        ...
        print(
            'Failed to load event updates: ${response.statusCode} - ${response.body}');
        return [];
      }
    } catch (e) {
