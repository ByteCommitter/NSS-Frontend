...
class _HomePageState extends State<HomePage> {
  ...
  void _connectAndListenToSocket() {
    ...
    _notificationSubscription =
        _socketNotificationService.notificationStream.listen((data) {
  ...
  Future<void> _fetchEvents() async {
    ...
    try {
      ...
      setState(() {
        ...
        _errorMessage =
            'Error fetching events: $e. Using sample events instead.';
      });
    }
  }
  ...
  Future<void> _fetchRegisteredEventsWithTimeout() async {
    ...
    try {
      ...
      setState(() {
        ...
        _registeredEvents = registeredApiEvents
            .map((e) => Event(
                  id: e.id,
                  title: e.title,
                  description: e.description,
        ...
        if (_registeredEvents.isNotEmpty) {
          print(
              'Successfully loaded ${_registeredEvents.length} registered events');
        } else {
      ...
      setState(() {
        ...
        _hasLoadingError = true;
        _errorMessage =
            'Error loading registered events. Some event details may be unavailable.';
      });
    }
  }
  ...
  Future<void> _fetchNotificationsWithRetry({int retryCount = 0}) async {
    ...
    try {
      ...
      print(
          'Successfully received ${notificationsData.length} notifications from API');
      ...
      if (retryCount < 2 && mounted && !_isDisposed) {
        ...
        if (mounted && !_isDisposed) {
          ...
          print(
              'Failed to load notifications after ${retryCount + 1} attempts');
        }
      }
    }
  }
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (_socketNotifications.isNotEmpty) ...[
                        _buildSectionHeader(
                            'Live Notifications', Icons.notifications_active),
                        const SizedBox(height: 16),
                      ...
                      _buildSectionHeader(
                          'Current Events', Icons.event_available_rounded),
                      const SizedBox(height: 16),
                      ...
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          _buildSectionHeader('Your Registered Events',
  ...
  DecorationImage _getDefaultEventImage(String title) {
    ...
    if (title.toLowerCase().contains('tree') ||
        title.toLowerCase().contains('plantation')) {
      imagePath = 'assets/images/tree_plantation.jpg';
    } else if (title.toLowerCase().contains('nss') ||
        title.toLowerCase().contains('meetup')) {
      imagePath = 'assets/images/nss_meetup.jpg';
    } else if (title.toLowerCase().contains('campus') ||
        title.toLowerCase().contains('cleanup')) {
        ...
        child: Image.network(
          ...
          errorBuilder: (context, error, stackTrace) {
            ...
            if (imageUrl.contains('imgur') ||
              ...
              print(
                  'Detected common image host, falling back to default design');
            }
  ...
  Widget _buildDefaultEventDesign(String title) {
    ...
    if (title.toLowerCase().contains('tree') ||
      ...
      eventIcon = Icons.eco;
    } else if (title.toLowerCase().contains('blood') ||
      ...
      eventIcon = Icons.favorite;
        ...
        title.toLowerCase().contains('teaching') ||
        title.toLowerCase().contains('workshop')) {
      ...
      eventIcon = Icons.school;
        ...
        title.toLowerCase().contains('nutrition')) {
      ...
      eventIcon = Icons.restaurant;
        ...
        title.toLowerCase().contains('awareness')) {
